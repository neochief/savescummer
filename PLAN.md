...
- Test the UI against a fake service that can produce busy, progress, failure, unavailable-snapshot and disconnected states. Cover the always-present Installed games section, scan progress, custom-game validation, narrow-width action wrapping, uninstalled custom rows and Forget confirmation/failure behavior.
...
[Save] [Cancel]
    ...
    Size Configure to its content and make it non-resizable, without unused child controls or blank areas. Keep icons off its primary Save/Cancel buttons. Across dialogs, visually accent the current default button while preserving safe defaults for destructive confirmations.
