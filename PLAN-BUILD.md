...
- **One task, "Launch at sign-in",** with `UsePreviousTasks=yes` and `checkedonce`: checked on first install; on upgrade the user's previous choice is kept, and a task added in a later version starts unchecked. A user who turned it off is never opted back in.
...
5. Installing needs no admin, puts the binaries under `%LOCALAPPDATA%\Programs\SaveScummer\bin`.
...
7. Installing over a running host shuts it down gracefully first.
8. Uninstalling removes the app and its own sign-in entry, never `%LOCALAPPDATA%\SaveScummer` or a moved checkpoint store.
