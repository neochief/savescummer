...
//!
//! - the three executables (two until the desktop exists) and what they need
...
use crate::frontend::{self, Desktop};
...
pub struct Inputs<'a> {
    pub mode: Mode,
    pub version: &'a str,
    /// Cargo's host and CLI executables (renamed when packaged).
    pub host: PathBuf,
    pub cli: PathBuf,
...
pub fn assemble(inputs: &Inputs) -> anyhow::Result<PathBuf> {
    ...
    if inputs.desktop.is_some() {
        ...
        if !has_files(&licenses) {
            bail!("the desktop's license texts are missing from {}", paths::show(&licenses));
        }
...
fn write_manifest(path: &Path, inputs: &Inputs) -> anyhow::Result<()> {
    let manifest = json!({
        ...
        "qt": inputs.desktop.map(|_| frontend::version()),
        "configuration": {
            "rust": match inputs.mode { Mode::Dev => "dev", Mode::Release => "release" },
            "desktop": inputs.desktop.map(|_| frontend::configuration(inputs.mode)),
        },
