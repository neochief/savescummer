...
- **A vanished DIR may be re-picked, by the catalog's rules.** The host stores each known game's DIR with the context the catalog chose it in (build, store account, when it was last seen) and passes it back on every scan. The catalog may then pick a different folder: when the DIR is confirmed missing and another candidate has newer saves (a game update moved them), or when the build or store account changed. An unreadable DIR (an unplugged drive) or a catalog update never moves it, and a DIR the user emptied stays until the game recreates it (PLAN-CATALOG.md, 4.5). The host accepts the catalog's answer. Checkpoints from the old DIR stay recorded but can't be restored into the new one; they become usable again if the game returns to the old folder. User overrides and custom games are never re-picked: a missing DIR there just makes Save, Load and Revert unavailable until it comes back or is reconfigured.
