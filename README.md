...
- `startup on|off` updates the current-user Windows sign-in registration and persisted
  ...
  or the installer task (checked on a first install so autostart works
  immediately; `checkedonce` presents it unchecked on upgrades, preserving an
  in-app opt-out, and uninstalling removes the entry only while it still
